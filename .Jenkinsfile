pipeline {
    ...
    stages {
        stage('Deploy') {
                agent { node 'identix' }
                    steps {
                        gitlabCommitStatus(name: "${STAGE_NAME}",builds: [[projectId: 'identix.space/identix-pass-service-ui', revisionHash: "${env.BRANCH_NAME}"],]){}
                        running("${STAGE_NAME}")
                        script {
                        ...
                        sh('''#!/bin/bash
                          ...
                          cd /var/www/identix-pass/$BRANCH_NAME/frontend
                          git pull
                          cd ../
